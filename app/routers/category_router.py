...
async def create_category(category_data: CategoryCreate, session: AsyncSession = Depends(get_session), current_user = Depends(get_current_admin)):
...
async def update_category(id: int, category_data: CategoryUpdate, session: AsyncSession = Depends(get_session), current_user = Depends(get_current_admin)):
    return await update_category_by_id(session=session, id=id, category_data=category_data)


@router.delete("/{id}", status_code=204)
async def delete_category(id: int, session: AsyncSession = Depends(get_session), current_user = Depends(get_current_admin)):
